...
| **M9** | Sim job model + freshness guard + runner (tunnel); netlist **dirty-check** + inputs **fingerprint** + real-spectre (plugin) | done — full job-loop joint test next |
| **M10** | Result schema 0.2 + provenance + sim-profile | planned (collab-led) |
